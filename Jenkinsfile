pipeline {
            ...
            steps {
                sh '''
                    ...
                    docker run --rm \
                        ...
                        python:3.11-slim \
                        sh -c "ls -la /workspace/performance/ && pip install --no-cache-dir locust==2.44.0 >/tmp/locust-install.log && locust -f /workspace/performance/locustfile.py --headless --users 20 --spawn-rate 5 --run-time 2m --csv /workspace/build/reports/locust/master --html /workspace/build/reports/locust/master.html"
                '''
            }
        }
    ...
    post {
        always {
                                sh -c "mkdir -p /workspace/performance && cat > /workspace/performance/locustfile.py <<'LOCUSTFILE'
        import os
        ...
        if __name__ == "__main__":
            ...
            pass
        LOCUSTFILE
         && pip install --no-cache-dir locust==2.44.0 >/tmp/locust-install.log && locust -f /workspace/performance/locustfile.py --headless --users 20 --spawn-rate 5 --run-time 2m --csv /workspace/build/reports/locust/master --html /workspace/build/reports/locust/master.html"
            sh '''
                set +e
